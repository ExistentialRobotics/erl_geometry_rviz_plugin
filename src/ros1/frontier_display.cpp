#include "erl_geometry_rviz_plugin/ros1/frontier_display.hpp"
...
namespace erl::geometry::rviz_plugin {
    ...
    FrontierDisplay::FrontierDisplay()
        ...
        m_topic_property_ = new rviz::RosTopicProperty(
            ...
            QString::fromStdString(
                ros::message_traits::datatype<erl_geometry_msgs::FrontierArray>()),
            "erl_geometry_msgs::FrontierArray topic to subscribe to.",
            this,
            SLOT(UpdateTopic()));

        m_color_mode_property_ = new rviz::EnumProperty(
            "Color Mode",
            "Uniform",
        ...
        m_alpha_property_ =
            new rviz::FloatProperty("Alpha", 1.0, "Transparency.", this, SLOT(UpdateAlpha()));
        m_alpha_property_->setMin(0.0);
        m_alpha_property_->setMax(1.0);

        m_line_width_property_ = new rviz::FloatProperty(
            "Line Width",
            2.0,
    ...
    void
    FrontierDisplay::UpdateQueueSize() {
        m_queue_size_ = m_queue_size_property_->getInt();
        Subscribe();
    }

    void
    FrontierDisplay::UpdateTopic() {
        Unsubscribe();
        reset();
        Subscribe();
    ...
    void
    FrontierDisplay::Subscribe() {
        if (!isEnabled()) { return; }

        try {
            Unsubscribe();
            const std::string &topic_str = m_topic_property_->getStdString();
            if (!topic_str.empty()) {
                m_sub_.reset(new message_filters::Subscriber<erl_geometry_msgs::FrontierArray>());
                m_sub_->subscribe(threaded_nh_, topic_str, m_queue_size_);
                m_sub_->registerCallback(
                    boost::bind(&FrontierDisplay::IncomingMessageCallback, this, _1));
            }
    ...
    void
    FrontierDisplay::IncomingMessageCallback(
        const erl_geometry_msgs::FrontierArray::ConstPtr &msg) {
        std::lock_guard<std::mutex> lock(m_mutex_);
...
PLUGINLIB_EXPORT_CLASS(erl::geometry::rviz_plugin::FrontierDisplay, rviz::Display)
